...
export class Product_addComponent implements OnInit {
  ...
  ngOnInit(): void {
    this.productForm = this.formBuilder.group({
      ...
      description: [''],
      // sizes: this.formBuilder.array([this.formBuilder.control('')]),
      // colors: this.formBuilder.array([this.formBuilder.control('')]),
      sizes: this.formBuilder.array([this.newSize()]),
      colors: this.formBuilder.array([this.newColor()]),
  ...
  getErrorMessage(fieldName: string) {
    ...
    // if (fieldName === 'sizes') {
    //   if (fieldControl?.hasError('required')) {
    //     return 'size không được phép để trống!';
    //   }
    // }
    // if (fieldName === 'colors') {
    //   if (fieldControl?.hasError('required')) {
    //     return 'colors không được phép để trống!';
    //   }
    // }
    return undefined;
  ...
  onSubmit() {
    ...
    this.productForm.value.sizes.forEach((size:any, index:any) => {
      formData.append(`sizes[${index}]`, size.size);
    });

    this.productForm.value.colors.forEach((color:any, index:any) => {
  ...
  newColor(): FormGroup {
    return this.formBuilder.group({
      color: ['', Validators.required] // yêu cầu phải nhập color
    });
  }
